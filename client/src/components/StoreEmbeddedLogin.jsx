...
import { Mail, Lock, Eye, EyeOff, Smartphone, Loader2, CheckCircle2 } from 'lucide-react';
...
export default function StoreEmbeddedLogin({ store }) {
    const navigate = useNavigate();
    const { loginWithEmail, sendOtp, verifyOtp, authConfig } = useStoreCustomer();
    ...
    const handleEmailLogin = async (e) => {
        ...
        try {
            ...
            // After successful login, context updates and checkout can proceed
        } catch (err) {
    ...
    return (
        <div className="w-full text-left bg-white rounded-3xl p-0 mt-4">
            {hasEmail && hasOtp && (
            ...
            {hasEmail && (
                <p className="text-center text-xs font-medium text-slate-500 mt-6 pt-4 border-t border-slate-100">
                    ...
                    <button onClick={() => {
                        // User can still go to register if they want, but let's navigate to the register route
                        navigate(getStoreRoute(store?.slug, `/account/register`));
                    }} className="font-bold hover:underline" style={{ color: themeColor }}>
                        Create Account
                    </button>
                </p>
            )}
