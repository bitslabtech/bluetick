...
const CreateTemplateModal = ({ isOpen, onClose, onSuccess, showToast }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Map the comprehensive formData down to what the backend currently expects.
        // Once the backend supports full payloads we will expand this.
        const payload = {
            name: formData.name,
            ...
            bodyVariables: Object.values(bodyVariables), // e.g. ["John", "Order #123"]
        ...
        try {
            ...
            const errMsg = err.response ? err.response.data.error : "Failed to create template";
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                disabled={selectedArchetype?.id === 'authentication'}
                                            >
                                    ...
                                    <div className="bg-white dark:bg-surface-dark rounded-2xl p-5 md:p-6 border border-slate-200 dark:border-white/5 shadow-sm space-y-5">
                                        ...
                                        {(formData.headerType === 'IMAGE' || formData.headerType === 'VIDEO' || formData.headerType === 'DOCUMENT') && (
                                            <div className="animate-in slide-in-from-top-2 duration-200 p-8 border-2 border-dashed border-slate-300 dark:border-white/20 rounded-xl flex flex-col items-center justify-center bg-slate-50 dark:bg-white/5 text-center">
                                                ...
                                                <h4 className="text-slate-900 dark:text-white font-semibold mb-1">Upload {formData.headerType.toLowerCase()} header</h4>
                                                <p className="text-sm text-slate-500 dark:text-text-secondary">Click or drag a file here to upload to Meta's servers.</p>
                                                <button type="button" className="mt-4 bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white px-4 py-2 rounded-lg text-sm font-semibold hover:border-primary transition-colors">
                                                    Select File
                                                </button>
                                            </div>
                                        )}
                                ...
                                {selectedArchetype?.id !== 'authentication' && selectedArchetype?.id !== 'carousel' && (
                                    <div className="pt-4 border-t border-slate-100 dark:border-white/5 space-y-4">
                                        <div className="flex justify-between items-center bg-slate-50 dark:bg-background-dark p-3 rounded-xl border border-slate-200 dark:border-white/10">
                                            ...
                                            {/* Logic: if there are ANY CTA buttons, you can't add QR. If there are ANY QR buttons, you can't add CTA */}
                                            <div className="flex items-center gap-2 overflow-x-auto pb-1 custom-scrollbar">
                                                <button type="button"
                                                    onClick={() => handleAddButton('QUICK_REPLY')}
                                                    disabled={formData.buttons.some(b => b.type !== 'QUICK_REPLY')}
                                                    className="px-3 py-1.5 text-xs font-bold bg-white dark:bg-surface-dark border border-slate-200 dark:border-white/10 rounded-lg hover:border-primary transition-colors disabled:opacity-50 flex items-center gap-1 shrink-0"
                                                >
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (formData.cards.length < 10) {
                                                        const newCards = [...formData.cards, { headerType: 'IMAGE', headerContent: '', content: '', buttons: [] }];
                                        ...
                                        {formData.cards[activeCardIndex] && (
                                            <div className="animate-in fade-in slide-in-from-top-2 p-4 border border-slate-200 dark:border-white/10 rounded-xl bg-slate-50/50 dark:bg-background-dark">
                                                ...
                                                <div className="mb-4">
                                                    ...
                                                    <div className="p-6 border-2 border-dashed border-slate-300 dark:border-white/20 rounded-xl flex flex-col items-center justify-center bg-white dark:bg-white/5 text-center transition-colors hover:border-primary">
                                                        <div className="bg-primary/10 text-primary p-2 rounded-full mb-3">
                                                            <ImageIcon className="w-5 h-5" />
                                                        </div>
                                                        <h4 className="text-slate-900 dark:text-white text-sm font-semibold">Upload Card Image/Video</h4>
                                                    </div>
                                                </div>
                                                ...
                                                {/* Card Buttons Logic goes here later based on Consistency Rules */}
                                                <div className="p-3 bg-blue-50/50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-lg flex items-start gap-2">
                                                    <AlertCircle className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                                                    <p className="text-xs text-blue-800 dark:text-blue-300">
                                                        Meta enforces strict button consistency. The button types (e.g., Quick Reply) configured roughly across all cards must match exactly.
                                                    </p>
                                                </div>
                                                ...
                                                <div className="flex gap-2 overflow-x-auto pb-2 snap-x custom-scrollbar">
                                                    {formData.cards.map((card, i) => (
                                                        <div key={`prev-card-${i}`} className="snap-center shrink-0 w-[80%] bg-white dark:bg-[#202c33] border border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden shadow-sm flex flex-col">
                                                            ...
                                                            {/* Card Body */}
                                                            <div className="p-2.5 text-[13.5px] leading-relaxed text-[#111b21] dark:text-[#e9edef] whitespace-pre-wrap break-words min-h-[60px]">
                                                            ...
                                                            <div className="mt-auto border-t border-slate-100 dark:border-white/5 w-full flex flex-col">
                                                                <div className="py-2.5 flex justify-center items-center text-[#00a884] text-[13px] font-medium w-full">
                                                                    <span className="text-[#00a884]/70">Standard Action</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ))}
                ...
                <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/80 dark:bg-transparent shrink-0 flex justify-between items-center backdrop-blur-sm">
                    ...
                    {step === 1 ? (
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all ${selectedArchetype
                                ? 'bg-primary text-white hover:bg-blue-600 shadow-lg shadow-primary/20 hover:shadow-primary/30 active:scale-95'
