        ...
        <div className="space-y-6 max-w-5xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button 
                    ...
                    className="w-full sm:w-auto px-4 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 flex items-center justify-center gap-2"
                >
                ...
                <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                    <table className="w-full text-left text-sm block md:table">
                        ...
                        <tbody className="divide-y divide-slate-100 block md:table-row-group">
                            {coupons.map(coupon => (
                                <tr key={coupon.id} className="block md:table-row hover:bg-slate-50 transition-colors p-4 md:p-0">
                                    <td className="px-0 md:px-6 py-2 md:py-4 block md:table-cell border-b border-slate-100 md:border-none mb-3 md:mb-0 pb-4 md:pb-4">
                                        <div className="flex items-center justify-between md:block">
                                            <span className="md:hidden text-xs font-semibold text-slate-500 uppercase tracking-wider">Code</span>
                                            <span className="font-bold font-mono px-2 py-1 bg-slate-100 rounded text-slate-800">
                                                {coupon.code}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-0 md:px-6 py-1.5 md:py-4 font-medium block md:table-cell">
                                        <div className="flex items-center justify-between md:block">
                                            <span className="md:hidden text-xs font-semibold text-slate-500 uppercase tracking-wider">Discount</span>
                                            <span>{coupon.discountType === 'percentage' ? `${coupon.discountValue}%` : `Fixed: ${coupon.discountValue}`}</span>
                                        </div>
                                    </td>
                                    <td className="px-0 md:px-6 py-1.5 md:py-4 text-slate-500 block md:table-cell">
                                        <div className="flex items-center justify-between md:block">
                                            <span className="md:hidden text-xs font-semibold text-slate-500 uppercase tracking-wider">Min Order</span>
                                            <span>{coupon.minOrderValue ? `> ${coupon.minOrderValue}` : 'None'}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-0 md:px-6 py-1.5 md:py-4 text-slate-500 block md:table-cell">
                                        <div className="flex items-center justify-between md:block">
                                            <span className="md:hidden text-xs font-semibold text-slate-500 uppercase tracking-wider">Expiry</span>
                                            <span>{coupon.expiresAt ? new Date(coupon.expiresAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Never'}</span>
                                        </div>
                                    </td>
                                    <td className="px-0 md:px-6 py-2 md:py-4 text-right block md:table-cell mt-3 md:mt-0 pt-3 md:pt-4 border-t border-slate-100 md:border-none">
                                        <div className="flex justify-end">
                                            <button onClick={() => handleDelete(coupon.id)} className="w-full md:w-auto flex items-center justify-center gap-2 p-2.5 md:p-2 bg-rose-50 text-rose-500 md:bg-transparent md:text-slate-400 hover:text-rose-600 rounded-xl md:rounded transition-colors font-bold text-xs md:font-normal">
                                                <Trash2 className="w-4 h-4 md:w-4 md:h-4" /> <span className="md:hidden">Delete Coupon</span>
                                            </button>
                                        </div>
                                    </td>
                                </tr>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm">
                    <div className="bg-white sm:rounded-2xl w-full h-full sm:h-auto max-w-md shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
                        <div className="px-4 md:px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                            <h3 className="font-bold text-lg text-slate-800">Create Promo Code</h3>
                            <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600 p-1">
                                <XCircle className="w-6 h-6" />
                            </button>
                        </div>
                        <form onSubmit={handleCreate} className="p-4 md:p-6 space-y-4 flex-1 flex flex-col overflow-y-auto">
                            <div>
                                <label className="block text-sm font-medium mb-1 text-slate-700">Coupon Code</label>
                                <input required type="text" value={form.code} onChange={e => setForm({...form, code: e.target.value.toUpperCase()})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl uppercase outline-none focus:ring-2 focus:ring-indigo-500" placeholder="e.g. SUMMER50" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-slate-700">Type</label>
                                    <select value={form.discountType} onChange={e => setForm({...form, discountType: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500">
                                        <option value="percentage">Percentage (%)</option>
                                        <option value="fixed">Fixed Amount</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1 text-slate-700">Discount Value</label>
                                    <input required type="number" step="0.01" value={form.discountValue} onChange={e => setForm({...form, discountValue: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" placeholder="10" />
                                </div>
                            </div>
                            <div>
                            ...
                            <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 mt-auto">
                                ...
                                <button type="submit" className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-indigo-600 text-white rounded-xl font-bold">Create Coupon</button>
                            </div>
                        </form>
                    </div>
                </div>
