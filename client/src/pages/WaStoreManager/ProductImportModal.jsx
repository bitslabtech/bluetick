...
function parseCSV(text) {
    // Strip UTF-8 BOM that Excel adds when saving CSV files (\uFEFF at start)
    ...
    // Normalize headers: strip BOM, trim whitespace, lowercase — so 'Name', 'NAME', '\uFEFFname' all become 'name'
    ...
    const rows = lines.slice(1).map(line => {
        ...
        headers.forEach((h, i) => {
            const raw = vals[i] ?? '';
            // Preserve internal content for description; trim whitespace edges for everything else
            obj[h] = h === 'description' ? raw.replace(/^[\s\r\n]+|[\s\r\n]+$/g, '') : raw.trim();
        });
        return obj;
    }).filter(r => Object.values(r).some(v => v !== ''));
    // Skip the hint/instruction row that our template generates (name col starts with REQUIRED: or Optional:)
    return rows.filter(r => {
        const nameVal = String(r.name || '').trim().toUpperCase();
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                // Use raw:false so numbers/dates come back as strings already formatted
                const rawRows = XLSX.utils.sheet_to_json(ws, { defval: '', raw: false });
                const parsed = rawRows.map(r => {
                    const clean = {};
                    Object.keys(r).forEach(k => {
                        // Normalize key: strip BOM, trim whitespace, lowercase — identical to CSV parser
                        const key = k.replace(/^\uFEFF/, '').trim().toLowerCase();
                        const raw = String(r[k] ?? '');
                        // Preserve internal content for description; trim edges for all others
                        clean[key] = key === 'description' ? raw.replace(/^[\s\r\n]+|[\s\r\n]+$/g, '') : raw.trim();
                    });
                    return clean;
                });
                // Skip hint/instruction rows the template generates
